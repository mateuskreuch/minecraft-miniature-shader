...
sunColor = isDay ? normalize(vec3(1.0 + clamp(66.0*(y - NOON)*(y - NOON) - 3.7142, 0.4, 1.0), 1.1, 1.0))
                   : vec3(0.04, 0.04, 0.12); 

// create transition between color presets using light height
sunColor *= clamp(2.0*0.1123*(gbufferModelViewInverse * vec4(shadowLightPosition, 1.0)).y - 1.0, 0.0, 1.0);
